...
def as_iarray(lu):
    lu = numpy.asarray(lu)
    return numpy.array(
        [(numpy.interval(lui[0], lui[1]) if lui[0] < lui[1] else numpy.interval(lui[1], lui[0]))
         for lui in lu.reshape(-1, 2)]
    ).reshape((lu.shape[0], lu.shape[1]))

def get_iarray(l, u):
    shape = l.shape
    return as_iarray(numpy.dstack((l, u))).reshape(shape)

is_iarray = lambda x: x.dtype == numpy.dtype(numpy.interval)
...
def from_cent_pert(cent, pert):
    ...
    pert = pert.reshape(-1)
    return numpy.array(
        [numpy.interval(cent[i] - pert[i], cent[i] + pert[i]) for i in range(len(cent))]
    ).reshape(shape)
